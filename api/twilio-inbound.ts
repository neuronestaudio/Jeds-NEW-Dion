...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const autoReplyEnabled = (process.env.TWILIO_AUTO_REPLY_ENABLED || 'false').toString().toLowerCase();
    const shouldAutoReply = ['true', '1', 'yes', 'on'].includes(autoReplyEnabled);
    const replyText = process.env.TWILIO_AUTO_REPLY_BODY || "Thanks, we received your message. We'll contact you shortly.";
